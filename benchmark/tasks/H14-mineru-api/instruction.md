# H14 · MinerU API Channel (host RPC contract + client plane, 0.1.1-era → 0.1.2-alpha.2)
...
I maintain a document-parsing tool plugin (working directory: `/app/fixture/`, i.e. the fixture directory inside the container) written in the 0.1.1-era style — it registers parsing tools on the host and serves its browser settings page through a dedicated `/mineru-api` RPC channel. The host has already been upgraded to dsh 0.1.2-alpha.2. Run the upgrade as a three-act drill:

1. **Diagnose** — inspect both planes (host RPC + browser half) against the 0.1.2-alpha.2 host, find every breakage (gateway facade, call contract, client inject list, peer cohort), and write the diagnosis with the corresponding card IDs to `/app/agent-output/H14-mineru-api/diagnosis.md`;
