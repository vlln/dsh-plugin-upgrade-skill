# M8 · Sidebar Brand Text (web UI slot plugin, 0.1.0-rc.8 cohort → 0.1.2-alpha.2)
...
I maintain a web UI plugin (working directory: `/app/fixture/`, i.e. the fixture directory inside the container) written in the 0.1.0-rc.8 cohort style — it replaces the sidebar's top-left brand name and revision badge with configurable text, edited live from a card in the Plugin Config page, and overrides `document.title` with the configured brand. The host has already been upgraded to dsh 0.1.2-alpha.2. Run the upgrade as a three-act drill:

1. **Diagnose** — inspect both planes (host settings/gateway + browser slot/store half) against the 0.1.2-alpha.2 host, find every breakage (snapshot-store engine, client context type, client inject list, peer cohort), attribute every inconsistency you find honestly (including ones that predate the upgrade), and write the diagnosis with the corresponding card IDs to `/app/agent-output/M8-brand-text/diagnosis.md`;
