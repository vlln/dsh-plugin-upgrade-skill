...
I maintain a composer-history plugin (working directory: `/app/fixture/`, i.e. the fixture directory inside the container) written in the 0.1.1-era style — an invisible dock entry collects recently sent prompts from the session's conversation nodes, and an ArrowUp/ArrowDown keydown listener feeds them back into the composer's textarea through the native value setter. The host has already been upgraded to dsh 0.1.2-alpha.2, where the composer surface and the session view internals have both changed. Run the upgrade as a three-act drill:

1. **Diagnose** — inspect the browser half against the 0.1.2-alpha.2 host, find every breakage (the composer surface, the keydown listener contract and its placement, the history collection source, the client inject list, the peer cohort), and write the diagnosis with the corresponding card IDs to `/app/agent-output/H16-history-dock/diagnosis.md`;
