...
I maintain a client plugin (working directory: `/app/fixture/`, i.e. the fixture directory inside the container) written in the 0.1.1-era style — it shadows the shipped tool views in the chat flow (`tool.call.toolview` slot) and merges consecutive same-tool calls into one card with compact child rows. The host has already been upgraded to dsh 0.1.2-alpha.2. Run the upgrade as a three-act drill:

1. **Diagnose** — inspect the plugin against the 0.1.2-alpha.2 host, find every breakage (the chat node package move, the card-derivation contract, the in-session read contract, the client inject list, the labels/override-dict contract, the peer cohort), and write the diagnosis with the corresponding card IDs to `/app/agent-output/H17-merge-calls/diagnosis.md`;
