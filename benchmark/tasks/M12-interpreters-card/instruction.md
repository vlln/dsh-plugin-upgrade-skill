# M12 · Interpreters Settings Card (host gateway + client card, 0.1.1-era → 0.1.2-alpha.2)
...
I maintain an interpreter-path tool plugin (working directory: `/app/fixture/`, i.e. the fixture directory inside the container) written in the 0.1.1-era style — the host half registers `run_python`/`run_node` tools and a self-hosted `/interpreters/api` HTTP gateway whose handler reads/writes the settings namespace through the settings bridge, and the browser half registers a settings-plugins card that converges on pushed connection invalidations. The host has already been upgraded to dsh 0.1.2-alpha.2. Run the upgrade as a three-act drill:

1. **Diagnose** — inspect both planes (host gateway + browser card) against the 0.1.2-alpha.2 host, find every breakage (the snapshot store's new package, the renamed settings type, the deleted client-runtime type sources, client inject list, peer cohort), and write the diagnosis with the corresponding card IDs to `/app/agent-output/M12-interpreters-card/diagnosis.md`;
